...
function meta(html:string,key:string){const safe=key.replace(/[.*+?^()|[\]\\]/g,"\\$&");const r=new RegExp('<meta[^>]+(?:property|name)=["\\']'+safe+'["\\'][^>]+content=["\\']([^"\\']+)["\\']',"i").exec(html);return r?.[1]||""}
...
export async function POST(req:Request){
 try{
  const body=await req.json();const url=body?.url;
  if(typeof url!=="string")return NextResponse.json({error:"A public URL is required."},{status:400});
  const parsed=new URL(url);if(!["http:","https:"].includes(parsed.protocol))throw new Error("Only public http(s) URLs are supported.");
  ...
  const description=meta(html,"og:description")||meta(html,"description");const imageUrls:string[]=[];const contexts:string[]=[];
  const re=/<(?:meta[^>]+(?:property|name)=["'](?:og:image|twitter:image)["'][^>]+content=["']([^"']+)["']|img[^>]+(?:src|data-src)=["']([^"']+)["'][^>]*)/gi;
  let m:RegExpExecArray|null;while((m=re.exec(html))&&imageUrls.length<12){const u=abs(res.url,m[1]||m[2]);if(u&&/^https?:/.test(u)&&!imageUrls.includes(u)){imageUrls.push(u);contexts.push("")}}
  const text=strip(html).slice(0,18000);const sentences=text.split(/(?<=[.!?])\s+/).filter((s:string)=>s.length>40).slice(0,8);
